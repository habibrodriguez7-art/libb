...
-- ============================================================
-- FIX: _createConfigTab sekarang menerima WindowObject
-- dan dibuat via AddTab agar terintegrasi dengan navigasi
-- ============================================================
function Library:_createConfigTab(WindowObject)
    -- Gunakan WindowObject:AddTab() agar sistem navigasi teregister dengan benar
    local configTab = WindowObject:AddTab({ Name = "Config", Icon = "settings" })
    ...
    autoSaveSection:AddToggle({
        ...
        Default  = true,
        NoSave   = true, -- toggle ini tidak perlu disimpan ke file config
    ...
    -- ── Tombol Reset — dua kali klik untuk konfirmasi ────────────
    -- PENTING: deklarasi local dulu SEBELUM callback didefinisikan
    -- agar closure bisa mereferensikan variabel yang sudah ada.
    local resetConfirm = false
    local resetThread  = nil
    local resetBtnFrame -- forward declare
    ...
    resetBtnFrame = self:CreateButton(mgmtSection._container, "Reset to Default", function()
        if not resetConfirm then
            ...
            ExecuteConfigCallbacks() -- sync semua UI ke nilai default
            self:MakeNotify({
                Title       = "Config",
    ...
    -- ── Tombol Delete — dua kali klik untuk konfirmasi ───────────
    local deleteConfirm = false
    local deleteThread  = nil
    local deleteBtnFrame -- forward declare
...
function Library:Window(config)
    ...
    function WindowObject:AddTab(tabConfig)
        ...
        function TabObject:AddSection(sectionTitle, isOpen)
            ...
            function SectionObject:AddButton(buttonConfig)
                ...
                if frame then frame.LayoutOrder = getNextLayoutOrder() end
                return frame -- FIX: return frame agar bisa diakses jika perlu
            end
    ...
    -- FIX: Panggil _createConfigTab dengan WindowObject sebagai argumen,
    -- SETELAH WindowObject selesai didefinisikan (termasuk fungsi AddTab-nya).
    self:_createConfigTab(WindowObject)
