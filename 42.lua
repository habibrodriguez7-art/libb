...
function Library:CreateButton(parent, label, callback)
    ...
    self:AddConnection("btn_" .. label .. tostring(button), button.MouseButton1Click:Connect(function()
        ...
        -- Jalankan callback di thread terpisah agar tidak membekukan UI / animasi
        if callback then
            task.spawn(function()
                pcall(callback)
            end)
        ...
        -- Gunakan task.delay alih-alih :Wait() yang menghentikan jalannya script
    ...
    function WindowObject:AddTab(tabConfig)
        ...
        function TabObject:AddSection(sectionTitle, isOpen)
            ...
            function SectionObject:AddParagraph(paragraphConfig)
                ...
                new("UICorner", {Parent = frame, CornerRadius = UDim.new(0, 4)})
                new("UIStroke", {Parent = frame, Color = colors.border, Thickness = 1, Transparency = 0.5})
                new("UIPadding", {
                    Parent = frame,
