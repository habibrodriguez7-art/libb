...
local guiParent = (function()
    -- Coba CoreGui dulu; kalau error (executor tidak support) fallback ke PlayerGui
...
function Library:CreateWindow(config)
    ...
    local existingGUI = guiParent:FindFirstChild(name)
    if existingGUI then
        existingGUI:Destroy()
        task.wait(0.1)
    end
...
function Library:Window(config)
    ...
    local _origAddTab = WindowObject.AddTab
    function WindowObject:AddTab(tabConfig)
        local result = _origAddTab(self, tabConfig)
        return result
    end

    local _origCreatePage = self.CreatePage
    local configTabCreated = false

    function WindowObject:AddTab(tabConfig)
        return _origAddTab(self, tabConfig)
    end
