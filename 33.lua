...
pcall(function()
    ...
    elseif get_hidden_gui then
        UIContainer = get_hidden_gui()
    end
end)
if not UIContainer then UIContainer = game:GetService("CoreGui") end
...
function Library:CreateWindow(config)
    ...
    self._currentPage = nil
    local existingGUI = UIContainer:FindFirstChild(name)
    if existingGUI then
        existingGUI:Destroy()
        task.wait(0.1)
    end
