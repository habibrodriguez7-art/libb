...
function Library:MakeNotify(config)
    ...
    self._maxActiveNotifs = self._maxActiveNotifs or 3
    while #self._activeNotifs >= self._maxActiveNotifs do
        local old = table.remove(self._activeNotifs, 1)
        pcall(function()
            if old and old.Parent then old:Destroy() end
        end)
    end
    local stackIndex = #self._activeNotifs
    local notif = new("Frame", {
        Parent = self._gui,
        Size = UDim2.new(0, 270, 0, 65),
        Position = UDim2.new(1, -280, 1, -75 - (stackIndex * 72)),
        BackgroundColor3 = colors.bg2,
        BackgroundTransparency = 0.05,
        BorderSizePixel = 0,
        ZIndex = 200
