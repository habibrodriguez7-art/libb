...
function Library:AddTab(options)
    ...
    local container = self:CreatePage(name, name, icon)
    
    local TabObj = {}
    function TabObj:AddSection(secTitle)
        local secName = type(secTitle) == "table" and (secTitle.Name or secTitle.Title) or secTitle
        local secContainer = Library:CreateCategory(container, secName, true)
        
        local SecObj = {}
        function SecObj:AddToggle(opt)
            return Library:CreateToggle(secContainer, opt.Title or opt.Name, opt.ConfigPath or opt.Title or opt.Name, opt.Callback, opt.NoSave, opt.Default)
        end
        function SecObj:AddButton(opt)
            return Library:CreateButton(secContainer, opt.Title or opt.Name, opt.Callback)
        end
        function SecObj:AddDropdown(opt)
            return Library:CreateDropdown(secContainer, opt.Title or opt.Name, opt.Icon, opt.Items or opt.Options, opt.ConfigPath or opt.Title or opt.Name, opt.Callback, opt.UniqueId, opt.Default)
        end
        function SecObj:AddMultiDropdown(opt)
            return Library:CreateMultiDropdown(secContainer, opt.Title or opt.Name, opt.Icon, opt.Items or opt.Options, opt.ConfigPath or opt.Title or opt.Name, opt.Callback, opt.UniqueId, opt.Default)
        end
        function SecObj:AddInput(opt)
            return Library:CreateInput(secContainer, opt.Title or opt.Name, opt.ConfigPath or opt.Title or opt.Name, opt.Default, opt.Callback)
        end
        function SecObj:AddTextBox(opt)
            return Library:CreateTextBox(secContainer, opt.Title or opt.Name, opt.Placeholder, opt.ConfigPath or opt.Title or opt.Name, opt.Default, opt.Callback)
        end
        return SecObj
    end
    return TabObj
