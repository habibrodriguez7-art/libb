...
local sidebarWidth = 108
local headerHeight = 32
local topBarHeight = 22
local rowHeight = 24
local fontSize = {
    title = 13,
    subtitle = 10,
...
function Library:CreateWindow(config)
    ...
    new("UIListLayout", {Parent = self._navContainer, Padding = UDim.new(0, 3), SortOrder = Enum.SortOrder.LayoutOrder})
    self._contentBg = new("Frame", {
        Parent = self._win,
        Size = UDim2.new(1, -(sidebarWidth + 6), 1, -(headerHeight + topBarHeight + 6)),
        Position = UDim2.new(0, sidebarWidth + 3, 0, headerHeight + 2),
        BackgroundTransparency = 1,
        BorderSizePixel = 0,
        ClipsDescendants = true,
        ZIndex = 4
    ...
    new("UICorner", {Parent = topBar, CornerRadius = UDim.new(0, 5)})
...
function Library:CreatePage(name, title, imageId, order)
    local page = new("Frame", {
        Parent = self._contentBg,
        Size = UDim2.new(1, -8, 1, -(topBarHeight + 8)),
        Position = UDim2.new(0, 4, 0, topBarHeight + 4),
        BackgroundTransparency = 1,
        BorderSizePixel = 0,
        Visible = false,
        ClipsDescendants = true,
    ...
    new("UIPadding", {Parent = contentContainer, PaddingTop = UDim.new(0, 2), PaddingBottom = UDim.new(0, 2), PaddingRight = UDim.new(0, 3)})
...
function Library:_switchPage(pageName)
    ...
    for name, data in pairs(self._navButtons) do
        ...
        data.btn.BackgroundTransparency = isActive and 0.45 or 1
...
function Library:CreateCategory(parent, title)
    ...
    local contentContainer = new("Frame", {
        ...
        Position = UDim2.new(0, 5, 0, rowHeight + 6),
        BackgroundTransparency = 1,
        Visible = false,
        AutomaticSize = Enum.AutomaticSize.Y,
        ZIndex = 7
    ...
    new("UIListLayout", {Parent = contentContainer, Padding = UDim.new(0, 3), SortOrder = Enum.SortOrder.LayoutOrder})
    new("UIPadding", {Parent = contentContainer, PaddingBottom = UDim.new(0, 5)})
    local isOpen = false
    self:AddConnection("categoryToggle_" .. title .. tostring(categoryFrame), header.MouseButton1Click:Connect(function()
        isOpen = not isOpen
        contentContainer.Visible = isOpen
        arrow.Rotation = isOpen and 180 or 0
        categoryFrame.BackgroundTransparency = isOpen and 0.45 or 0.55
    end))
    return contentContainer
end
function Library:CreateToggle(parent, label, configPath, callback, disableSave, defaultValue)
    local frame = new("Frame", {Parent = parent, Size = UDim2.new(1, 0, 0, rowHeight), BackgroundTransparency = 1, ZIndex = 7})
    new("TextLabel", {
        Parent = frame,
        Text = label,
    ...
    local function updateVisual()
        ...
        toggleCircle.Position = on and UDim2.new(1, -14, 0.5, -6) or UDim2.new(0, 2, 0.5, -6)
...
function Library:CreateButton(parent, label, callback)
    ...
    self:AddConnection("btn_" .. label .. tostring(button), button.MouseButton1Click:Connect(function()
        ...
        local tweenIn = TweenService:Create(btnFrame, TweenInfo.new(0.08, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
            BackgroundTransparency = 0,
            Size = UDim2.new(1, -4, 0, rowHeight - 2)
        })
        activeTween = tweenIn
        tweenIn:Play()
