...
function Library:_createInfoTab(WindowObject)
    ...
    local inviteLink = normalizeDiscordInviteLink(inviteRaw)
    local fallbackName = dc.Name or dc.Community or "Discord Community"
    ...
    local function applyStats(stats)
        ...
        if members ~= nil and online ~= nil then
    ...
    local function refreshDiscordStats()
        if inviteRaw == "" then return end
        task.spawn(function()
            local stats = fetchDiscordInviteStats(inviteRaw)
    ...
    local copyBtn = self:CreateButton(container, "COPY LINK", function()
        if inviteLink == "" then
            self:MakeNotify({
                Title = "Discord",
                Description = "Invite link belum diatur di Window config (Discord.Invite).",
                Color = colors.primary,
                Delay = 3,
            })
            return
        end
        if copyToClipboard(inviteLink) then
            self:MakeNotify({
                Title = "Discord",
            ...
            self:MakeNotify({
                Title = "Discord",
                Description = "Gagal menyalin. Link: " .. inviteLink,
                Delay = 4,
            })
        end
    end)
...
function Library:Window(config)
    ...
    if type(discordCfg) == "table" then
        ...
        self._discordInfo = {
            Invite = "meng-hub",
            Name = "Meng Hub | Peacefull Community",
            RefreshInterval = 45,
        }
    end
