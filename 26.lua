...
local function httpGet(url)
    ...
    if req then
        local ok, res = pcall(req, {
            Url = url,
            Method = "GET",
            Headers = {["User-Agent"] = "Roblox/DiscordInfo"},
        })
        if ok and res and (res.Success or res.StatusCode == 200) and res.Body and res.Body ~= "" then
            return res.Body
        end
    end
...
local function parseDiscordInviteCode(invite)
    if not invite or invite == "" then return nil end
    return invite:match("discord%.gg/([%w%-]+)")
        or invite:match("discord%.com/invite/([%w%-]+)")
        or invite:match("^([%w%-]+)$")
end
local function normalizeDiscordInviteLink(invite)
    if not invite or invite == "" then return "" end
    if invite:find("http://") or invite:find("https://") then
        return invite
    end
    if invite:find("discord.gg/") then
        return "https://" .. invite
    end
    local code = parseDiscordInviteCode(invite)
    return code and ("https://discord.gg/" .. code) or invite
end
local function fetchDiscordInviteStats(invite)
    local code = parseDiscordInviteCode(invite)
    if not code then return nil end
    local body = httpGet("https://discord.com/api/v10/invites/" .. code .. "?with_counts=true")
    ...
    return {
        name = data.guild and data.guild.name or nil,
        members = data.approximate_member_count,
        online = data.approximate_presence_count,
        invite = normalizeDiscordInviteLink(invite),
    }
end
local function fetchDiscordGuildWidget(guildId)
    guildId = tostring(guildId or ""):gsub("%s+", "")
    if guildId == "" then return nil end
    local body = httpGet("https://discord.com/api/guilds/" .. guildId .. "/widget.json")
    if not body then return nil end
    local ok, data = pcall(function()
        return HttpService:JSONDecode(body)
    end)
    ...
    return {
        ...
        invite = data.instant_invite and normalizeDiscordInviteLink(data.instant_invite) or nil,
    }
end
local function fetchDiscordServerStats(dc)
    if type(dc) ~= "table" then return nil end
    local stats = nil
    local guildId = tostring(dc.GuildId or dc.ServerId or dc.GuildID or ""):gsub("%s+", "")
    local invite = dc.Invite or dc.Link or ""
    if guildId ~= "" then
        stats = fetchDiscordGuildWidget(guildId)
    end
    if invite ~= "" then
        local inviteStats = fetchDiscordInviteStats(invite)
        if inviteStats then
            if stats then
                stats.members = inviteStats.members or stats.members
                stats.online = inviteStats.online or stats.online
                stats.name = stats.name or inviteStats.name
                stats.invite = stats.invite or inviteStats.invite
            else
                stats = inviteStats
            end
        end
...
function Library:Initialize()
    ...
    if self._pendingWindowObj then
        pcall(function()
            ...
            self:_createInfoTab(self._pendingWindowObj)
        end)
...
function Library:_createInfoTab(WindowObject)
    local dc = self._discordInfo or {}
    local inviteRaw = dc.Invite or dc.Link or ""
    local guildId = tostring(dc.GuildId or dc.ServerId or dc.GuildID or ""):gsub("%s+", "")
    local inviteLinkState = { url = normalizeDiscordInviteLink(inviteRaw) }
    local fallbackName = dc.Name or dc.Community or "Lynxx Official"
    local refreshInterval = tonumber(dc.RefreshInterval) or 45
    local communityIconId = dc.Icon or "rbxassetid://111416780887356"

    local infoTab = WindowObject:AddTab({ Name = "Info", Icon = "rbxassetid://6026563198" })
    local section = infoTab:AddSection("Community", true)
    local container = section._container
    ...
    new("UIStroke", { Parent = card, Color = colors.border, Thickness = 1, Transparency = 0.55 })
    new("UIPadding", {
        Parent = card,
    ...
    local communityIconSize = 30
    local headerRow = new("Frame", {
        Parent = card,
        Size = UDim2.new(1, 0, 0, communityIconSize),
        BackgroundTransparency = 1,
        LayoutOrder = 1,
        ZIndex = 8,
    })
    local iconHolder = new("Frame", {
        Parent = headerRow,
        Size = UDim2.new(0, communityIconSize, 0, communityIconSize),
        Position = UDim2.new(0, 0, 0, 0),
        BackgroundColor3 = colors.bg3,
        BackgroundTransparency = 0.35,
        ClipsDescendants = true,
    ...
    new("UICorner", { Parent = iconHolder, CornerRadius = UDim.new(0, 6) })
    new("ImageLabel", {
        Parent = iconHolder,
        Image = communityIconId,
    ...
    local nameLabel = new("TextLabel", {
        Parent = headerRow,
        Text = fallbackName,
        Size = UDim2.new(1, -(communityIconSize + 8), 1, 0),
        Position = UDim2.new(0, communityIconSize + 8, 0, 0),
        BackgroundTransparency = 1,
        Font = Enum.Font.GothamBold,
        TextSize = fontSize.normal,
        TextColor3 = colors.text,
    ...
    local function applyStats(stats)
        if stats and stats.name and stats.name ~= "" and not (dc.Name or dc.Community) then
            nameLabel.Text = stats.name
        end
        local members = stats and stats.members or nil
        local online = stats and stats.online or nil
        local membersText = members ~= nil and formatNumber(members) or "—"
        local onlineText = online ~= nil and formatNumber(online) or "—"
        if members ~= nil or online ~= nil then
            statsLabel.Text = formatRichText(string.format(
                'Members: <font color="#FAFAF9">%s</font>  <font color="#52525B">◆</font>  Online: <font color="#4ADE80">%s</font>',
                membersText,
                onlineText
            ))
        else
            statsLabel.Text = "Members: —  ◆  Online: —"
        end
    end

    local function refreshDiscordStats()
        if inviteRaw == "" and guildId == "" then return end
        task.spawn(function()
            local stats = fetchDiscordServerStats(dc)
            if stats and card.Parent then
                applyStats(stats)
                if stats.invite and stats.invite ~= "" then
                    inviteLinkState.url = stats.invite
                end
            end
        end)
    end
    ...
    local copyBtn = self:CreateButton(container, "COPY LINK", function()
        local link = inviteLinkState.url
        if link == "" then
            self:MakeNotify({
                Title = "Discord",
                Description = "Atur Discord.Invite atau Discord.GuildId di Library:Window({ Discord = {...} }).",
        ...
        if copyToClipboard(link) then
            self:MakeNotify({
                Title = "Discord",
                Description = "Link invite disalin ke clipboard!",
            ...
            self:MakeNotify({
                Title = "Discord",
                Description = "Gagal menyalin. Link: " .. link,
                Delay = 4,
            })
        end
    end)
...
function Library:Window(config)
    ...
    local discordDefaults = {
        Name = "Lynxx Official",
        Invite = "https://discord.gg/lynxx",
        GuildId = "1446770571835740322",
        Icon = "rbxassetid://111416780887356",
        RefreshInterval = 45,
    }
    local discordCfg = config.Discord or config.Info
    if type(discordCfg) == "table" then
        self._discordInfo = {
            Name = discordCfg.Name or discordCfg.Community or discordDefaults.Name,
            Invite = discordCfg.Invite or discordCfg.Link or discordDefaults.Invite,
            GuildId = discordCfg.GuildId or discordCfg.ServerId or discordCfg.GuildID or discordDefaults.GuildId,
            Icon = discordCfg.Icon or discordDefaults.Icon,
            RefreshInterval = discordCfg.RefreshInterval or discordDefaults.RefreshInterval,
        }
    else
        self._discordInfo = discordDefaults
    end
    ...
    function WindowObject:AddTab(tabConfig)
        ...
        if tabIcon and tabIcon ~= "" then
            if tabIcon:sub(1, 13) == "rbxassetid://" then
                iconId = tabIcon
            else
