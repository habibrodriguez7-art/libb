...
function Library:CreateWindow(config)
    ...
    new("UIStroke", {Parent = topBar, Color = colors.border, Thickness = 1, Transparency = 0.65})
...
function Library:CreateCategory(parent, title, startOpen)
    ...
    new("UICorner", {Parent = sectionBg, CornerRadius = UDim.new(0, 8)})
    new("UIStroke", {
        Parent = sectionBg,
        Color = colors.border,
        Thickness = 1,
    ...
    local contentList = new("UIListLayout", {Parent = contentContainer, Padding = UDim.new(0, 4), SortOrder = Enum.SortOrder.LayoutOrder})
    local isOpen = startOpen
    arrow.Rotation = startOpen and 180 or 0
    local function syncSectionBg()
        local h = categoryFrame.AbsoluteSize.Y
        if h > 0 then
            sectionBg.Size = UDim2.new(1, 0, 0, h)
        end
    end
    local function refreshScrollParent()
        local scroll = parent
        while scroll and not scroll:IsA("ScrollingFrame") do
            scroll = scroll.Parent
        end
        if not scroll then return end
        local pageList = scroll:FindFirstChildWhichIsA("UIListLayout")
        if pageList then
            local pad = scroll:FindFirstChildWhichIsA("UIPadding")
            local padY = 0
            if pad then
                padY = pad.PaddingTop.Offset + pad.PaddingBottom.Offset
            end
            scroll.CanvasSize = UDim2.new(0, 0, 0, pageList.AbsoluteContentSize.Y + padY)
        end
        local cp = scroll.CanvasPosition
        scroll.CanvasPosition = cp + Vector2.new(0, 1)
        scroll.CanvasPosition = cp
    end
    ...
    self:AddConnection("categoryLayout_" .. title .. tostring(categoryFrame), categoryList:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(refreshSectionLayout))
    self:AddConnection("categoryContent_" .. title .. tostring(categoryFrame), contentList:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(refreshSectionLayout))
    self:AddConnection("categorySize_" .. title .. tostring(categoryFrame), categoryFrame:GetPropertyChangedSignal("AbsoluteSize"):Connect(syncSectionBg))
    ...
    function WindowObject:AddTab(tabConfig)
        ...
        function TabObject:AddSection(sectionTitle, isOpen)
            ...
            function SectionObject:AddParagraph(paragraphConfig)
                ...
                local function measureLabel(label, text)
                    ...
                    label.Size = UDim2.new(1, 0, 0, math.max(bounds.Y, 14))
                end
                local function refreshParagraph()
                    if titleLabel.Visible and titleLabel.Text ~= "" then
                        measureLabel(titleLabel, titleLabel.Text)
                    else
                        titleLabel.Size = UDim2.new(1, 0, 0, 0)
                    end
                    measureLabel(contentLabel, contentLabel.Text)
                end
                frame:GetPropertyChangedSignal("AbsoluteSize"):Connect(refreshParagraph)
                task.defer(refreshParagraph)
                local paragraphObj = {
                    _frame        = frame,
                    _titleLabel   = titleLabel,
                    _contentLabel = contentLabel,
                    _refresh      = refreshParagraph,
