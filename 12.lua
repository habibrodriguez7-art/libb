...
function Library:CreateButton(parent, label, callback)
    local btnHeight = 28
    local btnIdleTransparency = 0.4
    local btnFrame = new("Frame", {
        Parent = parent,
        Size = UDim2.new(1, 0, 0, btnHeight),
        BackgroundColor3 = colors.primary,
        BackgroundTransparency = btnIdleTransparency,
        BorderSizePixel = 0,
        ZIndex = 8
    })
    ...
    local function setButtonHover(hovering)
        if clicking then return end
        btnFrame.BackgroundTransparency = hovering and 0.12 or btnIdleTransparency
        btnStroke.Transparency = hovering and 0.1 or 0.3
    end
    self:AddConnection("btnHoverIn_" .. label .. tostring(button), button.MouseEnter:Connect(function()
        setButtonHover(true)
    end))
    self:AddConnection("btnHoverOut_" .. label .. tostring(button), button.MouseLeave:Connect(function()
    ...
    self:AddConnection("btn_" .. label .. tostring(button), button.MouseButton1Click:Connect(function()
        ...
        local tweenIn = TweenService:Create(btnFrame, TweenInfo.new(0.08, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
            BackgroundTransparency = 0,
            Size = UDim2.new(1, -4, 0, btnHeight - 2)
        })
        activeTween = tweenIn
        tweenIn:Play()
